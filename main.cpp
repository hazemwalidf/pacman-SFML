...
int main()
{
	...
	menu mainmenu;
		...
		if (windowEvent.key.code == Keyboard::X)
		{
				...
				inputFile.close();
				Texture bricksTexture, pelletsTexture, background, livesTexture;
				...
				while (window.isOpen())
				{
					...
					if (timer.getElapsedTime().asMilliseconds() >= 350)
					{
						...
						cly.move(arr);
						ink.move(x.getD(), arr, x);
						pinky.move(x.getD(), arr, x);
						if (blink.eat(x)||cly.eat(x)||ink.eat(x)||pinky.eat(x))
						{
							break;
						}
					...
					cly.drawOnWindow(window);
					ink.drawOnWindow(window);
